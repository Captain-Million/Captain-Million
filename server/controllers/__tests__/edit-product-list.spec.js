...
test('add a new product', async (t) => {
  ...
  const inventory = await editProductList({
    editedProductNames,
    inventoryID,
    userID,
  });
...
test('remove a product also remove related document entries', async (t) => {
  const inventoryID = demoInventory._id;
  const userID = demoInventory.owners[0];
  const editedProductNames = demoInventory.products
    .map(prod => prod.name)
...
test('add and remove some products with trimmed names', async (t) => {
  const inventoryID = demoInventory._id;
  const userID = demoInventory.owners[0];
  const editedProductNames = demoInventory.products
    .map(prod => prod.name)
